...
const SALES_EMAIL = 'hello@sh-erp.com';
...
export function ContactSection() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const subject = encodeURIComponent(`Запит на демо SH ERP — ${name || 'без імені'}`);
    ...
    <section id="contact" className="border-t border-border py-20 sm:py-28">
      <div className="container">
        <div className="mx-auto grid max-w-4xl grid-cols-1 gap-12 lg:grid-cols-2">
          <Reveal>
            <h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">Готові побачити SH ERP у дії?</h2>
            <p className="mt-4 text-lg text-muted-foreground">
              Залиште контакти — ми зв&rsquo;яжемось і покажемо, як SH ERP закриє саме ваші процеси.
            </p>

            <div className="mt-8 space-y-4">
              <a href={`mailto:${SALES_EMAIL}`} className="flex items-center gap-3 text-sm hover:text-primary">
