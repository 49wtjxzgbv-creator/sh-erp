...
export async function buildImportReport(
  ...
  const entities: EntityReportLine[] = [];

  // --- Products: has its own exclusion reason (unresolved unit), tracked directly. ---
  ...
  const productDiff = await diffByLegacyId(prisma, companyId, 'product', productsIncluded.map((p) => p.legacyId));
  entities.push({ entity: 'products', total: graph.products.length, willCreate: productDiff.willCreate, willUpdate: productDiff.willUpdate, willSkip: productsExcluded });

  entities.push(await legacyIdEntityLine(prisma, companyId, 'supplier', 'suppliers', graph.suppliers.map((s) => s.legacyId)));
  entities.push(await legacyIdEntityLine(prisma, companyId, 'warehouse', 'warehouses', graph.warehouses.map((w) => w.legacyId)));
  entities.push(await legacyIdEntityLine(prisma, companyId, 'assembly', 'assemblies', graph.assemblies.map((a) => a.legacyId)));
  entities.push(await legacyIdEntityLine(prisma, companyId, 'customerOrder', 'customerOrders', graph.customerOrders.map((c) => c.legacyId)));
...
async function legacyIdEntityLine(
  ...
  const diff = await diffByLegacyId(prisma, companyId, model, legacyIds);
...
async function diffByLegacyId(
  ...
  const existing = await (prisma.tenant[model] as { findMany: (args: unknown) => Promise<{ legacyId: string | null }[]> }).findMany({
